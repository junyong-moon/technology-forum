...
app.post('/login', (req, res, next) => { 
    ...
    if (user) {
        req.logIn(user, function(err) {
            ...
            if (err) {
                res.render('login', {message: 'Please try again: An error occurred while processing your request'});
            }
            res.redirect('/');
        });
    } else {
...
app.post('/register', (req, res) => {
    ...
    User.register(new User({ username: newUsername, email: req.body.email, isAdmin: 0 }), 
        ...
        if (err) {
            res.render('register', {message: "Please try again: An error occurred while processing your request"});
        }
